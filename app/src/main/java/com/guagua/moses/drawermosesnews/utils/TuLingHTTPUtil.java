package com.guagua.moses.drawermosesnews.utils;
...
public class TuLingHTTPUtil {
    ...
    public TuLingHTTPUtil(String httpUrl, HashMap parameters,
                          String reqMethod) {
...
//                Log.e("TAG", "ss>>>>>>>>>>>>>>>>>>>>>" + ss);
    ...
    private String getHttpResponse() throws IOException {
         ...
         * GET 方式
         */
        if (reqMethod.equals(TuLingHTTPUtil.METHOD_GET)) {
            Iterator iterator = parameters.entrySet().iterator();//parameters >>>> "json":"{"account":"moses","password":"0716"}"
            StringBuilder obStringBuilder = new StringBuilder("?");
            while (iterator.hasNext()) {
                Map.Entry entry = (Map.Entry) iterator.next();
                String key = entry.getKey().toString();    //"account":"moses"
                String value = entry.getValue().toString(); //"password":"0716"
                obStringBuilder.append(key).append("=").append(value).append("&");
            }
            String obStr = obStringBuilder.substring(0, obStringBuilder.length() - 1);
             ...
             * POST方式
             */
        } else if (reqMethod.equals(TuLingHTTPUtil.METHOD_POST)) {
